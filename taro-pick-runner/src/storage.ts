...
export namespace Storage {
  ...
  export const mountContext = (extensionContext: vscode.ExtensionContext) =>
  ...
  export const saveData = (toSaveData: StorageData, overWrite = false) => {
    ...
    if (overWrite) {
      ...
      storageData = {
        pages: { ...(storageData.pages || {}), ...(toSaveData.pages || {}) },
        scenes: { ...(storageData.scenes || {}), ...(toSaveData.scenes || {}) },
        originConfigFile:
          toSaveData.originConfigFile || storageData.originConfigFile,
      }
    }
    ...
    if (_canSaveDataToProject) {
      const toSavePath: string = path.join(
        vscode.workspace.workspaceFolders![0].uri.fsPath,
        '.vscode',
        'taro-pick-runner.json'
      )
      !fs.existsSync(path.dirname(toSavePath)) &&
        fs.mkdirSync(path.dirname(toSavePath))
  ...
  export const loadData = () => {
    ...
    if (_canSaveDataToProject) {
      const toLoadPath: string = path.join(
        vscode.workspace.workspaceFolders![0].uri.fsPath,
        '.vscode',
        'taro-pick-runner.json'
      )
      ...
      try {
        ...
        vscode.window.showErrorMessage(
          `Taro-Pick-Runner load storage error: ${String(e)}`
        )
      }
    } else {
